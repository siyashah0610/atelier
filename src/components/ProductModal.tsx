      ...
      <div className="relative w-full sm:max-w-2xl bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] overflow-y-auto animate-slide-up">
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="sm:w-5/12 aspect-[3/4] sm:aspect-auto bg-stone-100 flex-shrink-0">
            <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover sm:rounded-l-2xl" />
          </div>
          ...
          <div className="flex-1 p-5 sm:p-6 space-y-4">
            ...
            {score !== undefined && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: getMatchColor(score) }}>
            ...
            <div>
              <p className="text-xs text-stone-400 uppercase tracking-widest font-medium mb-2">Color</p>
              <div className="flex gap-2">
                {product.hexColors.map((hex, i) => (
                  <div key={i} className="w-7 h-7 rounded-full border-2 border-white shadow" style={{ backgroundColor: hex }} />
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {product.tags.map((tag) => (
                <span key={tag} className="px-2.5 py-1 bg-stone-100 text-stone-600 text-[11px] rounded-full">{tag}</span>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <button
                ...
                className={`w-full py-3 rounded-xl text-sm font-semibold transition-all ${addedToCart ? 'bg-green-600 text-white' : 'bg-stone-900 text-white hover:bg-stone-800'}`}
              >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-colors ${saved ? 'border-rose-300 bg-rose-50 text-rose-600' : 'border-stone-200 text-stone-700 hover:border-stone-400'}`}
                >
                  {saved ? '♥ Saved' : '♡ Save'}
                </button>
                <a
                  ...
                  className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-stone-200 text-center text-stone-600 hover:border-stone-400 transition-colors"
                >
